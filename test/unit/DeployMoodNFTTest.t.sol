// SPDX-License-Identifier: SEE LICENSE IN LICENSE
pragma solidity ^0.8.19;

import {Test, console} from "forge-std/Test.sol";
import {DeployMoodNFT} from "../../script/DeployMoodNFT.s.sol";

contract DeployMoodNFTTest is Test {
    DeployMoodNFT deployer;

    function setUp() public {
        deployer = new DeployMoodNFT();
    }

    //to test svgToImageUri function is working fine
    function testConvertSvgToUri() public view {
        string memory expectedUri =
            "data:image/svg+xml;base64,";
        string memory svg = vm.readFile("./img/happy.svg");

        string memory svgToUri = deployer.svgToImageUri(svg);

        assert(keccak256(abi.encodePacked(svgToUri)) == keccak256(abi.encodePacked(expectedUri)));
    }
}
